...
import { CardInvoiceTable } from '../../../shared/ui-components/card-invoice-table/card-invoice-table';
...
@Component({
  ...
  imports: [RouterLink, CurrencyPipe, DatePipe, LucideAngularModule, ConfirmDialog, CardInvoiceTable],
...
export class CreditCardInvoice {
  ...
  // Abre o modal global de pagamento (ver PayInvoiceModalService/PayInvoiceModal) — o
  // mesmo fluxo também é acionável a partir de Transações do Mês (ver TransactionTable),
  // então a lógica de crédito por pagamento adiantado mora só lá, não duplicada aqui.
  payInvoice(): void {
    const cardId = this.creditCardId();
    ...
    this.payInvoiceModal.open({ creditCardId: cardId, year, month, mode: 'pay' });
  }

  editInvoicePaidDate(): void {
    const cardId = this.creditCardId();
    ...
    const { year, month } = this.yearMonth();
    this.payInvoiceModal.open({ creditCardId: cardId, year, month, mode: 'editDate' });
  }
